...
class NanFengSamplerAdvancedV2V:
    ...
    def _apply_second_pass_img_to_video_inplace(self, video_latent, i2v_image, i2v_vae, strength=1.0, stage_label=""):
        if i2v_image is None:
            self._log(
                "I2V",
                "二采图生视频已开启，但未连接“二采latent输入图像”，当前回退到普通二采前处理。",
                level="WARN",
                icon="⚠️",
        ...
        if i2v_vae is None:
            raise RuntimeError("开启“是否图生视频”后，需要连接“二采VAE”或“续写VAE”，因为二采前处理要调用 LTXVImgToVideoInplace。")
        ...
        i2v_result = self._sanitize_latent_noise_mask(i2v_result, prefer_video=True, reason="second_pass_img_to_video_inplace")
        self._log(
            "I2V",
            "已在二采前处理插入 LTXVImgToVideoInplace。",
            level="OK",
            icon="🖼️",
            i2v_stage=(stage_label or "default"),
            strength=float(strength),
    ...
    def _execute_second_pass_from_input(self, second_input_latent, guider, sampler, 添加噪波, 噪波种子, 二采西格玛, segment_label="", guider_source="引导器", sampler_source="采样器"):
        ...
        self._log("SAMPLE", "二采执行完成。", level="OK", icon="✅", segment=(segment_label or "full"))
        return second_result

    def _run_second_pass(self, first_pass_output, guider, sampler, 添加噪波, 噪波种子, 二采使用放大模型, 二采放大模型, 二采西格玛, 二采VAE=None, 二采引导器=None, 二采采样器=None, 是否图生视频=False, 二采latent输入图像=None, 二采图生视频VAE=None, 应用二采图生视频=False):
    ...
    def _run_delayed_segmented_second_pass(
        ...
        for pos, segment in enumerate(delayed_second_pass_segments, start=1):
            ...
            prepared_second_input = self._prepare_second_pass_input_latent(
                ...
                二采latent输入图像=二采latent输入图像,
                二采图生视频VAE=二采图生视频VAE,
                应用二采图生视频=bool(是否图生视频 and pos == 1),
            )
            ...
            segment_second_pass_guider = self._build_segment_cfg_guider(second_pass_guider_base, pos, total_segments)
            if pos == 1:
            ...
            self._log(
                ...
                level="INFO",
                icon="🎯",
                segment=f"{idx}/{total_segments}",
                overlap_v=overlap_v,
                overlap_a=overlap_a,
                input_shape=self._latent_shape_text(delayed_second_input),
                second_source_video_shape=self._latent_shape_text(second_video_source),
            )
            ...
            if pos < total_segments:
                ...
                self._log(
                    "SEG",
                    "下一段延后二采续写缓存已固定使用本段二采结果尾巴。",
                    level="INFO",
                    icon="🧬",
                    segment=f"{idx}/{total_segments}",
                    next_overlap_v=next_overlap_v,
                    next_overlap_a=(next_overlap_a if has_audio else 0),
                )
            else:
                previous_segment_video_output = None
                previous_segment_audio_output = None
    ...
    def _run_single_pipeline(self, 引导器, 采样器, latent_input, 一采西格玛文本, 添加噪波, 噪波种子, 启动二采, 二采使用放大模型, 二采放大模型, 二采西格玛, 西格玛=None, 二采引导器=None, 二采采样器=None, 二采VAE=None, segment_label="full", 返回一采结果=False, 是否图生视频=False, 二采latent输入图像=None, 二采图生视频VAE=None, 应用二采图生视频=False):
    ...
    def _run_segment_windows(
        ...
        if segment_use_delayed_segmented_second_pass:
            ...
            self._log(
                "SEG",
                "分段模式检测到“二采+放大模型”：当前将先对所有分段执行 0.5 一采；待全部一采完成后，再按段执行放大二采，并且二采阶段会使用上一段二采结果继续衔接。",
                level="INFO",
                icon="🪜",
        ...
        try:
            for pos, item in enumerate(mapped_windows, start=1):
                ...
                if 启动二采:
                    ...
                    segment_second_pass_guider = self._build_segment_cfg_guider(segment_second_pass_base, pos, len(mapped_windows))
                ...
                self._log(
                    ...
                    first_pass_video_shape=self._latent_shape_text(seg_video_source),
                    delayed_segmented_second_pass=segment_use_delayed_segmented_second_pass,
                )
                ...
                seg_result = self._run_single_pipeline(
                    ...
                    应用二采图生视频=bool(启动二采 and (not segment_use_delayed_segmented_second_pass) and 是否图生视频 and pos == 1),
                )
                ...
                if pos < len(mapped_windows):
                    ...
                    if segment_use_delayed_segmented_second_pass:
                        next_cache_message = "下一段一采续写缓存已固定使用本段 0.5 一采结果尾巴；待全部一采完成后，再按段执行二采，并在二采阶段使用上一段二采结果继续衔接。"
                    else:
                        next_cache_message = "下一段续写缓存已固定使用本段一采结果尾巴；二采结果仅用于当前段最终输出与拼接。"
        ...
        if segment_use_delayed_segmented_second_pass:
            self._log(
                "SEG",
                "分段 0.5 一采已全部完成，开始按段执行延后二采；二采阶段将使用上一段二采结果继续衔接下一段。",
                level="INFO",
                icon="🚀",
                first_pass_shape=self._latent_shape_text(final_output),
                upscale_model=二采放大模型,
            )
    ...
    def sample(
        ...
        if bool(启动二采):
            ...
            self._log(
                "FLOW",
                "二采链路已完成独立路由解析；未连接的二采端口会自动回退到一采端口。",
                level="INFO",
                icon="🧭",
        ...
        second_pass_img_to_video_vae = self._resolve_second_pass_img_to_video_vae(二采VAE=二采VAE, 续写VAE=续写VAE)
        if bool(是否图生视频):
            self._log(
                "I2V",
                "二采图生视频模式已开启：单段只补一次；分段只补二采首段；未连接图片时自动回退普通二采。",
                level="INFO",
                icon="🖼️",
                image_connected=(二采latent输入图像 is not None),
                vae_route=("二采VAE/续写VAE已就绪" if second_pass_img_to_video_vae is not None else "未连接二采VAE/续写VAE"),
            )
        ...
        if not mapped_windows:
            ...
            return self._run_single_pipeline(
                ...
                是否图生视频=是否图生视频,
                二采latent输入图像=二采latent输入图像,
                二采图生视频VAE=second_pass_img_to_video_vae,
                应用二采图生视频=bool(启动二采 and 是否图生视频),
            )
